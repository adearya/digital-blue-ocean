<div class="container detail-page mt-4 rounded bg-white">
  <div class="container pt-4 header-text">
    ...
    <p class="text-center mt-3"><br>{{$post->author->name}}APTISI Transactions on Technopreneurship (ATT), Vol 5. No 3. ISSN 2656-8888 (2023)</p>
    <h3>Abstract</h3>
  ...
  <div class="container information-text mt-5">
    ...
    <p><span class="fw-bold">KEYWORD : </span>{{$post->category->name}}, {{$post->category->name}}</p>
    <p><span class="fw-bold">VIEWS : </span>{{$post->views_count}}</p>
    <p><span class="fw-bold">DEPOSITING USER : </span>{{$post->author->name}}</p>
    <p><span class="fw-bold">DATE DEPOSITED : </span>{{$post->published_at}}</p>
  </div>
